...
function renderCaptionComponent(title, captionId, isBulk = false, accentColor = 'white') {
    if (!title || !title.trim()) {
        return '';
    }
        ...
        <div class="mt-4 mb-2 rounded-2xl bg-white/[0.04] border border-white/10 overflow-hidden transition-all duration-300">
            <div class="flex items-center justify-between p-3.5 cursor-pointer hover:bg-white/[0.07] transition-colors select-none" onclick="toggleCaption('${captionId}')">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    <button onclick="event.stopPropagation(); copyCaption('${captionId}', this)" class="px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white text-gray-300 hover:text-black text-xs font-medium transition-all flex items-center gap-1.5 cursor-pointer border border-white/10 shadow-sm" title="Copy text">
                    ...
                    <button type="button" class="p-1 rounded-lg hover:bg-white/10 text-gray-400 transition-colors" title="Toggle full text">
    ...
    <div class="mt-4 mb-2 p-4 rounded-2xl bg-white/[0.04] border border-white/10 relative group/caption transition-all hover:bg-white/[0.06]">
        <div class="flex items-center justify-between gap-2 mb-2.5 pb-2 border-b border-white/[0.06]">
            ...
            <button onclick="copyCaption('${captionId}', this)" class="px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white text-gray-300 hover:text-black text-xs font-medium transition-all flex items-center gap-1.5 cursor-pointer border border-white/10 shadow-sm" title="Copy text">
                <svg class="w-3.5 h-3.5 icon-copy" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/>
                </svg>
                <svg class="w-3.5 h-3.5 icon-check hidden text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function createResultCard(data, isBulk = false) {
    ...
    const captionId = 'tw_' + (data.id || Math.random().toString(36).substring(2, 9));
                    ...
                    <div class="flex-grow min-w-0">
                        ...
                        <div class="relative aspect-video rounded-xl overflow-hidden bg-gray-900 mb-4 border border-gray-800">
                            ...
                            <div class="absolute inset-0 flex items-center justify-center">
                                <button onclick="triggerDownload('${downloadUrl}', this)" class="bg-white text-black p-4 rounded-full shadow-lg hover:scale-110 transition-transform cursor-pointer">
                        ...
                        ${renderCaptionComponent(title, captionId, isBulk, 'white')}
                        ...
                        <button onclick="triggerDownload('${downloadUrl}', this)" class="w-full bg-white hover:bg-gray-200 text-black font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2 shadow-lg cursor-pointer">
